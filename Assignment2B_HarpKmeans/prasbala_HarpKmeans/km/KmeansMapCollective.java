...
public class KmeansMapCollective  extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		if (args.length < 6) {
			System.err.println("Usage: KmeansMapCollective <numOfDataPoints> <num of Centroids> " + "<size of vector> <number of map tasks> <number of iteration> <workDir> <localDir>");
	...
	void launch(int numOfDataPoints, int numCentroids, int sizeOfVector, int numMapTasks, int numIteration, String workDir, String localDir) throws IOException, URISyntaxException, InterruptedException, ExecutionException, ClassNotFoundException {
		...
		if (!fs.mkdirs(outDir)) {
	    		throw new IOException("Unable to create " + outDir.toString());
	    	}
		...
		runKMeansAllReduce(numOfDataPoints,numCentroids, sizeOfVector, numIteration, JobID, numMapTasks, configuration, workDirPath, dataDir, cenDir, outDir);
		long endTime = System.currentTimeMillis();
		System.out.println("Execution Time: "+ (endTime - startTime));
	}

	private void runKMeansAllReduce(int numOfDataPoints, int numCentroids, int vectorSize, int numIterations, int JobID, int numMapTasks, Configuration configuration, Path workDirPath, Path dataDir, Path cDir, Path outDir) throws IOException,URISyntaxException, InterruptedException,ClassNotFoundException {
		System.out.println("Starting All Reduce Job");
		long jobSubmitTime;
		...
		do {
			...
			Job kmeansJob = configureKMeansJob(numOfDataPoints, numCentroids, vectorSize, numMapTasks, configuration, workDirPath, dataDir,cDir, outDir, JobID, numIterations);
			...
			System.out.println("end Jod#" + JobID + " " + new SimpleDateFormat("HH:mm:ss.SSS").format(Calendar.getInstance().getTime()));
			System.out.println("| Job#"+ JobID + " Finished in " + (System.currentTimeMillis() - jobSubmitTime)+ " miliseconds |");
			if (!jobSuccess) {
	...
	private Job configureKMeansJob(int numOfDataPoints, int numCentroids, int vectorSize, int numMapTasks, Configuration configuration,Path workDirPath, Path dataDir, Path cDir, Path outDir, int jobID, int numIterations) throws IOException, URISyntaxException {
